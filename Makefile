...
ifeq ($(ENABLE_CLANG),1)
	...
	ENABLE_LTO := 0
	# -ffat-lto-objects makes clang emit LLVM bitcode alongside the machine
	# code, which ld then hands to GCC's lto1 and which dies with
...
#	May need to adjust this to match your system. On macOS with homebrew's
#	gcc-arm-none-eabi the sysroot is inside the Cellar, so override it:
#	  make ENABLE_CLANG=1 ARM_SYSROOT=$(brew --prefix)/Cellar/gcc-arm-none-eabi/*/arm-none-eabi
	ARM_SYSROOT ?= /usr/arm-none-eabi
	CC = clang --sysroot=$(ARM_SYSROOT) --target=arm-none-eabi
...
ifeq ($(ENABLE_CLANG),1)
	# Even with no LTO anywhere, ld's plugin still tries to run lto1 over
	# clang's objects because of the .llvm_addrsig section they carry. Linking
	# is done by the GCC driver, so tell it there is no LTO to do.
	LDFLAGS += -fno-lto
endif
