...
void Main(void)
{
    ...
    */
    ...
    #ifdef ENABLE_FEAT_F4HWN_RESUME_STATE
        // The range variables only exist when ENABLE_SCAN_RANGES is on, so restoring a
        // range on boot needs its own guard rather than riding on the resume-state one.
